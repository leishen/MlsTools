...
def parse_house(xmlstr):
    ...
    mlsnum = x.getElementsByTagName('MLSNumber')[0]
    price = x.getElementsByTagName('Price')[0]
    status = x.getElementsByTagName('ListingStatus')[0]
    taxes = x.getElementsByTagName('Taxes')[0]
    ...
    h = house.House(MLS=getnodetext(mlsnum), 
                    Price=util.format_number(getnodetext(price)), 
                    Address=address, 
                    HOA=util.format_number(hoa), 
                    Status=getnodetext(status),
                    Taxes=util.format_number(getnodetext(taxes)))
